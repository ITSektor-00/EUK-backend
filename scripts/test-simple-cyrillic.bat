...
curl -X GET "http://localhost:8080/api/test-basic-pdf" -H "Accept: application/pdf" --output test-cyrillic-simple.pdf

if exist test-cyrillic-simple.pdf (
    echo.
    echo ✅ PDF je uspešno generisan: test-cyrillic-simple.pdf
    echo.
    echo Otvaranje PDF-a...
    start test-cyrillic-simple.pdf
) else (
    echo.
