...
curl -X GET "http://localhost:8080/api/test-envelope-pdf" -H "Accept: application/pdf" --output test-koverat-final-fixed.pdf

if exist test-koverat-final-fixed.pdf (
    echo.
    echo ✅ PDF je uspešno generisan: test-koverat-final-fixed.pdf
    echo.
    ...
    start test-koverat-final-fixed.pdf
) else (
    echo.
