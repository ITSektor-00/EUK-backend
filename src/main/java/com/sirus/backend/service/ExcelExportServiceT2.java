...
@Service
public class ExcelExportServiceT2 {
    
    private static final Logger logger = LoggerFactory.getLogger(ExcelExportServiceT2.class);
    private static final String TEMPLATE_PATH = "excelTemplate/ЕУК-T2.xlsx";
    ...
    public byte[] generateExcelWithAllData() throws Exception {
        logger.info("Starting Excel T2 export with all data");
        
        List<EukUgrozenoLiceT2> allData = ugrozenoLiceT2Repository.findAll();
        logger.info("Loaded {} T2 records for export", allData.size());
    ...
    public byte[] generateExcelWithFilteredData(List<Integer> ids) throws Exception {
        logger.info("Starting Excel T2 export with {} filtered records", ids.size());
        
        List<EukUgrozenoLiceT2> filteredData = ugrozenoLiceT2Repository.findAllById(ids);
        logger.info("Loaded {} T2 records for export", filteredData.size());
     ...
     */
    private byte[] generateExcel(List<EukUgrozenoLiceT2> data) throws Exception {
        // Učitaj template
        ...
        logger.info("T2 Template loaded successfully");
        ...
        for (int i = 0; i < data.size(); i++) {
            EukUgrozenoLiceT2 entity = data.get(i);
            ...
            fillRowData(row, entity, i + 1);
            currentRow++;
        }
        
        logger.info("T2 Data written to {} rows", data.size());
        
        // Obriši sve redove nakon podataka
        int lastRowNum = sheet.getLastRowNum();
        if (lastRowNum >= currentRow) {
            logger.info("Clearing T2 rows from {} to {}", currentRow, lastRowNum);
            ...
            logger.info("Cleared {} T2 rows after data", (lastRowNum - currentRow + 1));
        }
        ...
        logger.info("Excel T2 export completed successfully");
    ...
    private void cacheTemplateStyles(Row templateRow) {
        ...
        logger.info("Cached {} T2 cell styles from template", cellCount);
    }
    ...
    /**
     * Popunjava jedan red podacima T2
     */
    private void fillRowData(Row row, EukUgrozenoLiceT2 entity, int redniBroj) {
        ...
        setCellValue(row, 8, entity.getEdBroj()); // I - ED broj
        setCellValue(row, 9, entity.getPokVazenjaResenjaOStatusu()); // J - Pokriće važenja rešenja
    }
    ...
    public String generateFileName() {
        ...
        return "EUK_T2_Izvestaj_" + date + ".xlsx";
    }
}
