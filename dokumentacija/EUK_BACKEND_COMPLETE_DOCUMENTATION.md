...
| Metoda | Endpoint | Opis | Parametri | Autentifikacija |
|--------|----------|------|-----------|-----------------|
| `GET` | `/` | Dohvatanje svih korisnika (sa paginacijom i filterima) | `page`, `size`, `role`, `isActive`, `search` | ❌ |
| `GET` | `/{id}` | Dohvatanje korisnika po ID-u | `id` path | ❌ |
| `GET` | `/username/{username}` | Dohvatanje korisnika po username-u | `username` path | ❌ |
| `GET` | `/email/{email}` | Dohvatanje korisnika po email-u | `email` path | ❌ |
| `GET` | `/role/{role}` | Dohvatanje korisnika po roli | `role` path | ❌ |
| `GET` | `/active` | Dohvatanje aktivnih korisnika | - | ❌ |
| `GET` | `/inactive` | Dohvatanje neaktivnih korisnika | - | ❌ |
| `GET` | `/count` | Broj ukupnih korisnika | - | ❌ |
| `GET` | `/count/active` | Broj aktivnih korisnika | - | ❌ |
...
- **User listing** - dohvatanje svih korisnika sa filterima
- **User search** - pretraga po username, email, ime, prezime
- **Role-based filtering** - filtriranje po korisničkim rolama
- **Status filtering** - filtriranje po aktivnosti korisnika
...
#### **User (Korisnik):**
- **Fields:** username, email, firstName, lastName, role, isActive, createdAt, updatedAt
- **Security:** passwordHash (nikad se ne vraća preko API-ja)
- **Roles:** USER, ADMIN, MODERATOR (proširivo)
- **Status:** isActive (true/false)
- **Relationships:** može biti povezan sa EukPredmet
...
- **User listing** - dohvatanje svih korisnika sa paginacijom
- **User search** - pretraga po username, email, firstName, lastName
- **Role-based filtering** - filtriranje po korisničkim rolama
- **Status filtering** - filtriranje po aktivnosti korisnika
- **User statistics** - broj ukupnih i aktivnih korisnika
